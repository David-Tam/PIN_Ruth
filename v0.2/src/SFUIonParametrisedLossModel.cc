...
#include "SFUIonParametrisedLossModel.hh"
...
SFUIonParametrisedLossModel::SFUIonParametrisedLossModel(
    ...
    cutEnergies(0) 
{
  ...
  AddDEDXTable("ICRU73",
	       new SFUIonStoppingData("ion_stopping_data/icru73"),
...
G4double SFUIonParametrisedLossModel::MinEnergyCut(
                                       const G4ParticleDefinition*,
...
G4double SFUIonParametrisedLossModel::MaxSecondaryEnergy(
...
G4double SFUIonParametrisedLossModel::GetChargeSquareRatio(
                             const G4ParticleDefinition* particle,
...
G4double SFUIonParametrisedLossModel::GetParticleCharge(
                             const G4ParticleDefinition* particle,
			     const G4Material* material,
                             G4double kineticEnergy) {   // Kinetic energy 
...
void SFUIonParametrisedLossModel::Initialise(
                                       const G4ParticleDefinition* particle,
  ...
  size_t nmbCouples = coupleTable -> GetTableSize();
    ...
    G4cout << "SFUIonParametrisedLossModel::Initialise():"
...
G4double SFUIonParametrisedLossModel::ComputeCrossSectionPerAtom(
                                   const G4ParticleDefinition* particle,
                                   G4double kineticEnergy,
  ...
  G4cout << "########################################################"
         << G4endl
         << "# SFUIonParametrisedLossModel::ComputeCrossSectionPerAtom"
         << G4endl
...
G4double SFUIonParametrisedLossModel::CrossSectionPerVolume(
				   const G4Material* material,
                                   const G4ParticleDefinition* particle,
                                   G4double kineticEnergy,
                                   G4double cutEnergy,
...
G4double SFUIonParametrisedLossModel::ComputeDEDXPerVolume(
                                      const G4Material* material,
			              const G4ParticleDefinition* particle,
				      G4double kineticEnergy,
				      G4double cutEnergy) {
...
void SFUIonParametrisedLossModel::PrintDEDXTable(
...
void SFUIonParametrisedLossModel::PrintDEDXTableHandlers(
                   const G4ParticleDefinition* particle,  // Projectile (ion) 
                   const G4Material* material,  // Absorber material
                   G4double lowerBoundary,      // Minimum energy per nucleon
                   G4double upperBoundary,      // Maximum energy per nucleon
...
void SFUIonParametrisedLossModel::SampleSecondaries(
  ...
  do {
    ...
    if(g > 1.0) {
        G4cout << "SFUIonParametrisedLossModel::SampleSecondary Warning: "
               << "Majorant 1.0 < "
...
void SFUIonParametrisedLossModel::UpdateRangeCache(
                     const G4ParticleDefinition* particle,
...
void SFUIonParametrisedLossModel::UpdateDEDXCache(
                     const G4ParticleDefinition* particle,
                     const G4Material* material,
                     G4double cutEnergy) {
...
void SFUIonParametrisedLossModel::CorrectionsAlongStep(
                             const G4MaterialCutsCouple* couple,
  ...
  G4cout << "########################################################"
         << G4endl
         << "# SFUIonParametrisedLossModel::CorrectionsAlongStep"
         << G4endl
         << "# cut(MeV) = " << cutEnergy/MeV 
         << G4endl;
...
void SFUIonParametrisedLossModel::BuildRangeVector(
                     const G4ParticleDefinition* particle,
                     const G4MaterialCutsCouple* matCutsCouple) {
...
#ifdef PRINT_DEBUG_DETAILS
      G4cout << "SFUIonParametrisedLossModel::BuildRangeVector() bin = " 
             << i <<", E = "
             << energy / MeV << " MeV, R = " 
             << range / mm << " mm"
             << G4endl;     
...
G4double SFUIonParametrisedLossModel::ComputeLossForStep(
                     const G4MaterialCutsCouple* matCutsCouple,
                     const G4ParticleDefinition* particle,
                     G4double kineticEnergy,
                     G4double stepLength) {
...
#ifdef PRINT_DEBUG
     G4cout << "SFUIonParametrisedLossModel::ComputeLossForStep() range = " 
...
G4bool SFUIonParametrisedLossModel::AddDEDXTable(
                                const G4String& nameX,
                                G4VIonDEDXTable* table, 
                                G4VIonDEDXScalingAlgorithm* algorithm) {

  if(table == 0) {
     G4cerr << "SFUIonParametrisedLossModel::AddDEDXTable() Cannot "
  ...
  G4IonDEDXHandler* handler = 
                      new G4IonDEDXHandler(table, scalingAlgorithm, nameX); 
...
G4bool SFUIonParametrisedLossModel::RemoveDEDXTable(
				 const G4String& nameX) {

	G4cout << "Removing stopping power dataset: " << nameX << G4endl;
  LossTableList::iterator iter = lossTableList.begin();
  LossTableList::iterator iter_end = lossTableList.end();
...
void SFUIonParametrisedLossModel::DeactivateICRU73Scaling() {

  RemoveDEDXTable("ICRU73");
  AddDEDXTable("ICRU73", new SFUIonStoppingData("ion_stopping_data/icru73"));
}
